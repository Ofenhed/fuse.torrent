...
import Utils (getBestHash, wrapStdString, wrapSharedArray', wrapVec')

import Control.Concurrent.STM (readTVarIO, atomically, readTVar, TVar, newTVarIO, swapTVar, modifyTVar')
import Control.Monad (void, when)
import Control.Monad.IO.Class (MonadIO(liftIO))
import Data.Default (Default(def))
import Foreign (Ptr, peek, nullPtr, alloca)
import Foreign.Concurrent (newForeignPtr)
import Foreign.C.String (peekCString)
import Foreign.ForeignPtr (ForeignPtr, withForeignPtr, finalizeForeignPtr, touchForeignPtr)
...
alertReadPiece = do
  ...
  liftIO $ alloca $ \size -> alloca $ \piece -> alloca $ \buf -> do
    arr <- [CU.block| boost::shared_array<char>* {
...
alertTorrent = do
  p <- getPtr
  handle <- liftIO [CU.block| lt::torrent_handle* {
...
alertTorrentDeletedHash = do
  ...
  if isValid == 0
    then return Nothing
        ...
        let deleted_handler = [C.funPtr| lt::info_hash_t alert_hash(lt::alert *alert) {
            return static_cast<lt::torrent_deleted_alert*>(alert)->info_hashes;
        } |]
            removed_handler = [C.funPtr| lt::info_hash_t alert_hash(lt::alert *alert) {
...
alertErrorMsg = do
  p <- getPtr
  msg <- liftIO [CU.block| std::string* {
      if (auto torrent_alert = dynamic_cast<lt::torrent_alert*>($(lt::alert* p))) {
